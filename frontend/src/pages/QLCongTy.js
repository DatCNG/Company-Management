...
const QLCongTy = () => {
  ...
  const openAdd = () => { setModalType("add"); setModalData(null); setShowModal(true); };
  const openView = (data = null) => { setModalType("view"); setModalData(data); setShowModal(true); };
  const openEdit = (data = null) => { setModalType("edit"); setModalData(data); setShowModal(true); };

  const renderModalContent = () => {
    if (modalType === "add")
      return <ThemCongTy onClose={() => setShowModal(false)} />;
    if (modalType === "view") return <XemCongTy data={modalData} />;
    if (modalType === "edit")
      return <SuaCongTy data={modalData} onClose={() => setShowModal(false)} />;
    return null;
  };
  ...
  const modalTitle =
    modalType === "add" ? (
      <div className="quanly-title qlct-title">
        <h2><IoAddCircle style={{ marginRight: ".3rem" }} /> Thêm Công Ty</h2>
      </div>
    ) : modalType === "view" ? (
      <div className="quanly-title qlct-title">
        <h2><FaEye style={{ marginRight: "0.3rem" }} /> Chi Tiết Công Ty:</h2>
        <h2>-</h2>
      </div>
    ) : modalType === "edit" ? (
      <div className="quanly-title qlct-title">
  ...
  return (
    <div className="quanly-content">
      <div className="quanly-container qlct-container">
        {/* Tiêu đề chính */}
        <div className="quanly-title qlct-title">
          <h2><FaWarehouse style={{ marginRight: "0.3rem" }} />Công Ty</h2>
        </div>

        <div className="quanly-title-sub">
          <div className="ct-tong">
            <div className="quanly-title-sub-content">
              <h3>Tổng công ty:<span className="tong">-</span></h3>
            </div>
          </div>
          <div className="ct-hoatdong">
            <div className="quanly-title-sub-content">
              <h3>Hoạt động:<span className="hoatdong">5</span></h3>
            </div>
          </div>
          <div className="ct-tamngung">
            <div className="quanly-title-sub-content">
              <h3>Tạm ngừng:<span className="tamngung">5</span></h3>
            </div>
          </div>
        </div>
        ...
        <div className="timvaloc timvaloc-border">
          <div className="tim">
            <input type="text" className="tim-input" placeholder="Tìm theo mã, tên, địa chỉ,..." />
          </div>
          <div className="loc">
            <select className="loc-select"><option value="">-- Lọc theo tên công ty --</option></select>
            <select className="loc-select"><option value="">-- Lọc theo địa chỉ --</option></select>
          </div>
        </div>
            ...
            <table className="quanly-table">
              ...
              <tbody className="quanly-tbody qlct-tbody" ref={tbodyRef}>
                  <tr>
                    ...
                    <td>Cần Thơ</td>
                    <td>
                      <button className="button-xem quanly-button-xem" onClick={() => openView()}>
                        <FaRegEye />
                      </button>
                      <button className="button-sua quanly-button-sua" onClick={() => openEdit()}>
                        <FiEdit2 />
                      </button>
        ...
        {showModal &&
          createPortal(
            <div
              ...
              role="dialog"
              aria-modal="true"
              onClick={(e) => { if (e.target.classList.contains("ql-overlay")) setShowModal(false); }}
            >
              <div className="overlay-content">
                <div className="overlay-header">
                  <h3 className="quanly-title qlct-title">
                    <span style={{ display: "flex", alignItems: "center" }}>{modalTitle}</span>
                  </h3>
                  <button className="overlay-close" aria-label="Đóng" onClick={() => setShowModal(false)}>
